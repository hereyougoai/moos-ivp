...
void showSynopsis()
{
  ...
  blk("  as TRAIL_UPDATE_<VNAME> messages -- by default as an offset  ");
  blk("  from the intruder heading (station_frame=relative), so the    ");
  blk("  station sweeps smoothly when it turns; station_frame=absolute ");
  blk("  orders the same arc as a compass bearing instead.             ");
  blk("                                                                ");
...
void showExampleConfigAndExit()
{
  ...
  blk("  spread_deg  = 90               // default, total arc spread  ");
  blk("  trail_range = 25               // default, meters            ");
  blk("  cant_deg    = 0                // default; degrees each USV's ");
  blk("                                 // BOW is turned IN off the    ");
  blk("                                 // parallel with the target    ");
  blk("                                 // (0 = bows parallel; max 90) ");
  blk("                                 //   (live: EVICT_CANT)        ");
  blk("  cant_dead_deg = 5              // default; no cant for a      ");
  blk("                                 // station this close to dead  ");
  blk("                                 // ahead/astern of the target  ");
  ...
  blk("  station_frame   = relative     // default, or: absolute      ");
  blk("  formation_var   = EVICT_FORMATION // default; operator switch ");
  blk("                                                                ");
...
void showInterfaceAndExit()
{
  ...
  blk("  EVICT_CANT        = 25          (degrees of inward bow cant    ");
  blk("                                   off the parallel; 0 = none)   ");
  blk("  DEPLOY_ALL        = true                                      ");
  blk("  TARGET_ALERT_ALL  = true        (operator INTERCEPT/SURVEY)   ");
  blk("  EVICT_FORMATION   = block | herd | stern   (operator menu)    ");
  blk("                    = mode=herd,spread=150,trail_range=30       ");
  blk("                    = mode=herd,cant=25                          ");
  blk("                      (switches formation mid-run; the side     ");
  blk("                       being left is released first)            ");
  blk("                                                                ");
  blk("PUBLICATIONS:                                                   ");
  blk("------------------------------------                            ");
  blk("  TRAIL_UPDATE_<VNAME> = trail_angle=.. # trail_angle_type=..   ");
  blk("                         # trail_range=.. # trail_cant=..       ");
  blk("                         (trail_cant is SIGNED: + starboard of  ");
  blk("                          the target's course, - port, so that   ");
  blk("                          cant_deg always reads as inward)       ");
