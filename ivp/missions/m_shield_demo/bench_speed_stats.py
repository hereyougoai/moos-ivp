                ...
                if float(count_str) >= 1.0:
                    ...
                    if first_engage is not None:
                        ...
                        if t_start in tgt:
                            ...
                            x0, x1, y0, y1 = REGION
                            px, py = tgt[t_start][0], tgt[t_start][1]
...
def main():
    if len(sys.argv) < 2:
        print("Usage: bench_speed_stats.py <speed_group_or_run_dirs...>")
        print("Example: bench_speed_stats.py bench_results/spd_*")
        return
    ...
    for arg in sys.argv[1:]:
        ...
        if runs:
            ...
            for r in runs:
                res = analyse_run(r)
                if res:
                    speed_groups[grp_name].append(res)
        else:
            res = analyse_run(arg)
            if res:
                parent = os.path.basename(os.path.dirname(arg.rstrip("/\\")))
                speed_groups[parent].append(res)
    ...
    print("=" * 105)
    print("                      M_SHIELD_DEMO 目標船速變因模擬統計分析報告")
    print("=" * 105)

    summary_table = []

    for grp in sorted(speed_groups.keys()):
        runs = speed_groups[grp]
        n = len(runs)
        if n == 0:
            continue
        ...
        evict_times = [r["first_evict_time"] for r in runs if r["first_evict_time"] is not None]
        mean_1st_evict = sum(evict_times) / len(evict_times) if evict_times else None
        ...
        mean_push_pct = sum(r["push_pct"] for r in runs) / n
        mean_out_pct = sum(r["outside_pct"] for r in runs) / n
        mean_min_rng = min(r["min_rng_eng"] for r in runs)
        total_unsafe_s = sum(r["unsafe_s"] for r in runs)

        summary_table.append({
            "group": grp,
            "trials": n,
            "success_trials": n_success,
            "succ_rate": succ_rate,
            "mean_evic": mean_evic,
            "mean_1st_eng": mean_1st_eng,
            "mean_1st_evict": mean_1st_evict,
            "mean_evict_dur": mean_evict_dur,
            "mean_post_dist": mean_post_dist,
            "mean_tortuosity": mean_tortuosity,
            "mean_push": mean_push,
            "mean_push_pct": mean_push_pct,
            "mean_out_pct": mean_out_pct,
            "mean_min_rng": mean_min_rng,
            "total_unsafe_s": total_unsafe_s
        })

        print(f"\n[實驗組: {grp}] (試驗次數: {n} 輪)")
        print(f"{'Run':<8} {'成功?':<8} {'驅逐次數':<10} {'接戰耗時':<10} {'驅離耗時':<10} {'逃逸航行距離':<14} {'路徑迂迴比':<12} {'PUSH':<8} {'minR(m)':<8}")
        print("-" * 105)
        for r in runs:
            run_name = os.path.basename(r["run_dir"])
            succ_str = "SUCCESS" if r["success"] else "FAIL"
            t_eng = f"{r['first_engage']:.1f}s" if r["first_engage"] is not None else "-"
            ...
            print(f"{run_name:<8} {succ_str:<8} {r['evictions']:<10d} {t_eng:<10} {t_dur:<10} {d_post:<14} {tort:<12} {r['push_mean']:<8.1f} {r['min_rng_eng']:<8.1f}")

    print("\n" + "=" * 115)
    print("                                      總體橫向對比匯總表")
    print("=" * 115)
    print(f"{'目標速度組':<13} {'成功率':<9} {'平均驅逐次數':<13} {'包夾驅離耗時':<13} {'目標逃逸航行距離':<18} {'路徑迂迴比(Tortuosity)':<24} {'平均壓迫力(PUSH)':<15}")
    print("-" * 115)
    for s in summary_table:
        ...
        print(f"{s['group']:<13} {s['succ_rate']:>5.1f}%    {s['mean_evic']:<13.2f} {t_dur:<13} {d_post:<18} {tort:<24} {s['mean_push']:<15.1f}")
